...
loadData = (example) => {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      } else enableElement(document.getElementById("execute_btn"));
    },
  });
};
...
showExecutionResults = (result) => {
  ...
  if (result["num_filtered_epochs"]) showStatus(`${result["num_filtered_epochs"]} epochs are filtered out`);
  ...
  getCombinedTable = (data_tab1, data_tab2) => {
    makeHTMLRow = (last_epoch, i, data_comb) => {
      let html_row = "";
      if (i != last_epoch) html_row = `<td>${last_epoch}-${i}</td>`;
      else html_row = `<td>${last_epoch}</td>`;
  ...
  if (result["cut_options"] && result["cut_options"].length > 0) {
    ...
    $.get(
      ...
      function (data) {
        ...
        for (let i = 0; i < cut_options.length; i++) {
          let checked = "";
          if (cut_options_selected.find((x) => x == cut_options[i]) != undefined) checked = "checked";
    ...
    let msg =
      'We have found cut options for long NREM period. Please \
            <a class="link-primary" href="#" onclick="activateTab(\'nremp_cut_options\');">click here</a>\
                to show the options.';
    document.getElementById("nremp_cut_option_msg").innerHTML = `<div class="alert alert-info alert-dismissible fade show text-center" style="font-size:0.8rem">\
                ${msg}\
...
execute = () => {
  ...
  if (save_cut_options) {
    for (let i = 0; i < save_cut_options.length; i++) {
      if (save_cut_options[i].checked) fd.append("nremp_cut_options_selected", save_cut_options[i].value);
    }
  }
...
selectAllChannels = (value) => {
  ...
  select_all_or_none_btn.onclick = () => {
    selectAllChannels(!value);
  };
};
...
moveAnnotsToLeftPanel = () => {
  ...
  for (var i = 0; i < checkboxes_selected.length; i++) {
    let annot = checkboxes_selected[i].value;
    annots_right_settings[sleep_stage_selected].splice(
      annots_right_settings[sleep_stage_selected].findIndex((val) => val === annot),
      1
    );
  }
...
loadRightPanel = () => {
  ...
  for (const [sleep_stage, annots] of Object.entries(annots_right_settings)) {
    ...
    for (var i in annots) {
      annot = annots[i];
      let disabled = "";
      if (sleep_stage != sleep_stage_selected) disabled = "disabled";
      html += `<div class="row"> \
                <div class="col-1"> \
      ...
      if (annots_selected == "") annots_selected = annot;
      else annots_selected += ", " + annot;
    }
...
loadSleepStageSettings = () => {
  hasKeyword = (x) => {
    for (var i in keywords) if (x.toLowerCase().search(keywords[i]) >= 0) return true;
    return false;
  };

  var keywords = ["sleep stage", "wake stage", "stage wake", "w stage", "stage w", "wake", "n1 stage", "stage n1", "nrem 1", "n1", "stage1", "stage 1", "n 1", "n2 stage", "stage n2", "nrem 2", "n2", "stage2", "stage 2", "n 2", "n3 stage", "stage n3", "nrem 3", "n3", "stage3", "stage 3", "n 3", "rem", "r stage", "stage r"];
  ...
  for (var i in annots_all_settings) {
    ...
    if (
      Object.values(annots_right_settings).find((annots) => {
        for (var i in annots) if (annots[i] == annot) return true;
        return false;
      }) != undefined
    )
      continue;
    ...
    if (hasKeyword(annot)) html_relevant += html;
    else html_nonrelevant += html;
  }
  if (html_relevant == "") html_relevant = '<label class="pb-1">No relevant sleep stage available for selection</label>';
  else html_relevant = '<label class="pb-1">Relevant sleep stage events</label>' + html_relevant;

  if (html_nonrelevant != "") html_nonrelevant = '<label class="pb-1">Other events</label>' + html_nonrelevant;
...
loadSettings = () => {
  ...
  for (var i in channel_settings) {
    ...
    for (var j in channel_row) {
      ...
      html_cols += `<div class="col-6 col-md-3 col-xl-2 border border-light">\

...
class SleepInfo:
    ...
    # --------------------------------------------------------------------------
    def __init__(self, sample_name, edf_file_path=None, annot_file_path=None, input_file_type=None, use_cache=True, logger=logger):
    ...
    def loadEEG(self):
        ...
        self.logger.printDec('Loading eeg data')
    ...
    def applyFilterOnEEG(self):
        ...
        if len(indices) == 0:
            raise SpecialError("All epochs are filtered out !!! Nothing to show...")
        ...
        self.logger._print(f'{self.num_filtered_epochs} epochs were filtered out')
    ...
    def extractEpochsFromAnnots(self):
        ...
        whole_annotations = np.concatenate([self.annotations.onset[:, None].astype(object), self.annotations.duration[:, None].astype(object),
                                           self.annotations.description[:, None].astype(object)], axis=1)  # , dtype=object)
        ...
        for t, d, desc in whole_annotations:
            ...
            if desc.lower() not in Config.sleep_stage_event_to_id_mapping:
                if desc in bad_annots:
                    bad_epochs |= {i for i in range(epoch_id, epoch_id + n_epochs + 1)}
                continue
    ...
    def extractEpochs(self):
        ...
        self.logger.printDec('Extracting epoch wise data')
    ...
    def extractSpectogram(self, fmin=0, fmax=30, nw=2):
        ...
        if not (self.spectogram is None or self.spec_freq is None):
            return
        ...
        if self.eeg_data_epoch_wise is None:
            self.extractEpochs()
        if self.sampling_freq is None:
            self.loadEEG()

        self.logger.printDec('Extracting spectogram')
        ...
        if self.enable_cache or True:
            np.savez(spectogram_path, spec=self.spectogram, spec_freq=self.spec_freq)
        ...
        def getCutOptionsPerPeriod(sleep_stage_durations, start_epoch, duration_non_w):
            ...
            if len(index_n3) > 0:
                ...
                for i, (stage, duration) in enumerate(sleep_stage_durations):
                    ...
                    if stage != Config.WAKE_STAGE:
                        duration_non_w_before += duration
        ...
        for [sc_id, sc], duration in sleep_period_durations:
            ...
            if sc == 'NREMP' and duration > self.durationInEpoch(120):
                ...
                if duration_without_w > self.durationInEpoch(120):
                    ...
                    if len(cut_options):
                        self.logger._print(f'NREMP (period {sc_id}) can be cut at {cut_options[:]}')
    ...
    def extractSleepPeriods(self):
        ...
        def groupSleepPeriodsWithDuration(sleep_stage_durations):
            ...
            for i, (st, d) in enumerate(sleep_stage_durations):
                ...
                if st == Config.WAKE_STAGE:
                    if d >= self.durationInEpoch(5) or (i == 0 or i == len(sleep_stage_durations)-1):
                        sc = 'NA'
                    else:
                        sc = sc_prev
                elif st == Config.REM_STAGE:
                    sc = 'REMP'
                elif st in [Config.N1_STAGE, Config.N2_STAGE, Config.N3_STAGE]:
                    sc = 'NREMP'
                else:
                    sc = 'NA'
                ...
                if i == len(sleep_stage_durations)-1:
                    consecutive_periods.append([sc_prev, total_d])
            ...
            if len(self.cut_options_selected) > 0:
                ...
                for i in range(self.cut_options_selected[j]-1, len(self.sleep_periods)):
                    if j < len(self.cut_options_selected) and i == self.cut_options_selected[j]-1:
                        sc_index_inc += 1
                        j += 1
                    if self.sleep_periods[i][1] == 'NREMP':
                        self.sleep_periods[i][0] = self.sleep_periods[i][0][:2] + str(int(self.sleep_periods[i][0][2:]) + sc_index_inc)
        ...
        self.logger.printDec('Extracting sleep periods')
    ...
    def generatePlot(self, sleep_stage_levels, sleep_stage_colors=None, title='', fig_path=None):
        ...
        self.logger.printDec('Generating plot')

        if sleep_stage_colors is None:
            sleep_stage_colors = {stage: 'k' for stage in sleep_stage_levels}
        ...
        if self.shift_len > 0:
            title += f' (shifted {self.shift_len} epochs)'
        ...
        if not (self.sleep_periods is None):
            ...
            for i in range(len(self.sleep_periods)):
                ...
                if stage_sc != 'NA':
                    if index_overlay_anchor is None:
...
def loadSleepData(input_file_path, output_folder_path, input_file_type="edf", app_logger=None):
    ...
    if input_file_type == 'edf':
        ...
        logger._print(f'No valid input provided !!!')
        return

    logger.printDec('Processing ' + file_name)
    ...
    sleep_obj = SleepInfo(sample_name=sample_name, edf_file_path=edf_file_path, annot_file_path=annot_file_path, input_file_type=input_file_type, use_cache=False, logger=logger)
...
def extractSleepStages(sleep_obj, apply_filter=False):
    ...
    if sleep_obj.input_file_type == 'edf':
        ...
        sleep_obj.logger._print('Invalid input file type !!!')
